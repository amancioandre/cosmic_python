...
class TestChangedBatchQuantity:
    ...
    def test_changes_available_quantity(self):
        ...
        messagebus.handle(
            commands.ChangeBatchQuantity("batch1", 50), uow
        )
    ...
    def test_reallocates_if_necessary(self):
        uow = FakeUnitOfWork()
        event_history = [
            commands.CreateBatch("batch1", "INDIFERENT-TABLE", 50, None),
            commands.CreateBatch("batch2", "INDIFERENT-TABLE", 50, date.today()),
            commands.Allocate("order1", "INDIFERENT-TABLE", 20),
            commands.Allocate("order2", "INDIFERENT-TABLE", 20),
        ]
        ...
        messagebus.handle(
            commands.ChangeBatchQuantity("batch1", 25), uow
        )
